...
ECS_template_create_register() {
    ...
    then
        log "No ECS reserved memory defined. Going with default value 1024 MB"
...
check_service_status() {
    ...
    do
        ...
        if [[ $counter -gt $COUNTER_LIMIT ]] ; then
            echo "Service did not reach steady state with in $(($COUNTER_LIMIT*15+60)) seconds. Please check"
            exit 1
        fi
    done
    echo "$servicestatus"
