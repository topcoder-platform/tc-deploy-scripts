...
#nk- added
 ...
 validate_file_json_values()
 {
            ...
            IFS=$o  
         echo "$(tput setaf 2)================Valid json syntax value==========$(tput setaf 7)"
...
user_information()
{
   # echo "Next Step"
   circleadd1=""
    echo -e "\n$(tput setaf 4)NEXT STEP \n- Navigate to your app Git Repo, Append the following 2 line at .circleci/config.yml of your Git repo\n$(tput setaf 7)"
    echo "---1. Sample .circle/config.yml is created, Edit the file-----------------------------------------"
    echo "---2. Search comment '#masterscript line 1 and line 2 add(ed) here' and append---------------------"
    echo -e "\n---3. COPY LINE 1 AS SHOWN BELOW\n"

    #cp SampleConfigFiles/Sample-Nodejs.yml Sample_NodeCircleci.yml
    ...
    echo $(tput setaf 3)$circleadd1$(tput setaf 7)
    echo -e "\n---3. COPY LINE 2 AS SHOWN BELOW"
    { echo $(tput setaf 3)
        echo -e "- deploy:" 
        echo -e "    command: |" 
        echo -e "      echo 'ntegration testing'" 
        echo -e "      cp ./../buildscript/master_deply_v4.1.sh ."  
        echo -e "      ./master_deply_v3.sh -e DEV -t \$CIRCLE_SHA1 -s $FILENAMEBUF" 
       echo $(tput setaf 7)
    }
        sed -i "" "s|#msadd1|$circleadd1|" Sample-Nodejs.yml
        sed -i "" "s|#msadd2|$circleadd2|" Sample-Nodejs.yml

    echo -e "\n---3. For reference, these 2 lines are appended at config.yml. Check other project info before check-in---"
    echo -e "\n---4. FINAL STEP, Update your Branch details and Check-in. MASTER SCRIPT automatically triggers. Monitor Circleci"
}
...
case $opt in
    ...
    2 ) echo "call modify fun"
        ...
        ;;  
    3 ) 
        echo "$(tput setaf 3)Ensure your input json file is valid & adheres to EBS_var_template.json or ECS_var_template.json schema$(tput setaf 7)"
